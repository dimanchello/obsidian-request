...
export function formatAndHighlightResponseBody(
    ...
    if (contentType?.includes('json') || text.trim().startsWith('{') || text.trim().startsWith('[')) {
        try {
            ...
            const highlighted = highlightJsonText(jsonStr)
    ...
    if (
        ...
        (text.includes('xml') || contentType?.includes('xml') || contentType?.includes('html'))
    ) {
        let formatted = ''
        let pad = 0
